...
Equipped with billions of parameters, \gls{llm} demonstrate human-level performance across a wide range of tasks, including question answering, summarization, classification, and text generation, and they are now being extensively deployed in both industrial and public sectors.
Despite these remarkable achievements, several critical challenges remain unresolved.
A central issue lies in how effectively and efficiently these models can adapt to new information and evolving knowledge, while simultaneously overcoming the resource and technical constraints inherent in this process.
...
However, as the scale of \gls{llm} grows exponentially, retraining the entire set of parameters demands substantial time, GPU memory, and computational resources.
This limitation becomes particularly impractical in environments where real-time adaptability is required or where data continuously flows in, since repeatedly retraining the full model is unsustainable from a maintenance perspective.
To address these challenges, recent research has explored a variety of approaches that can integrate up-to-date information into model outputs without resorting to full fine-tuning.
...
Representative approaches include Transfer Learning, which leverages pretrained knowledge; Continual Learning, which enables sequential adaptation; Prompt Engineering (e.g., zero-shot, few-shot, and in-context learning), which adjusts model behavior through tailored prompts; \gls{peft}, which emphasizes efficiency in parameter updates; \gls{rag}, which integrates external knowledge retrieval; and \gls{kme}, which locally modifies specific knowledge within the model.
%%표 삽입?
...
Amid these challenges, a recently emerging approach that has attracted considerable attention is \gls{kme}.
\gls{kme} provides a methodology for efficiently modifying the knowledge base of \gls{llm}, enabling the incorporation of specific knowledge without retraining the entire model. This approach enhances the reliability of \gls{llm} responses, facilitates the integration of up-to-date information, and better addresses practical demands.

In particular, \gls{kme} is often conceptualized in three phases inspired by human learning processes: Recognition, Association, and Mastery.

The Recognition Phase involves retrieving or injecting external knowledge so that the model can recognize new information, with \gls{rag} and in-context learning serving as representative techniques.The Association Phase connects new knowledge to the model's representation space through methods such as \gls{lora}and adapters. Finally, the Mastery Phase directly modifies model parameters to fully internalize knowledge, with approaches such as ROME, MEMIT, and MEND representing this category.

These KME techniques are generally evaluated using two primary criteria: (1) Generality—whether the edited knowledge is applied consistently across diverse input scenarios, and (2) Locality—whether the editing process avoids unintended influence on unrelated knowledge.
...
Considering the diverse limitations of existing approaches, there is a pressing need for structural flexibility that can evaluate the logical relationship between newly acquired information and prior knowledge, and selectively apply fine-tuning strategies according to the identified relation type.
Motivated by this perspective, this study proposes a novel learning strategy called \gls{raft}, which aims not to indiscriminately overwrite or erase prior knowledge, but rather to achieve both consistent knowledge retention and selective updating.
...
The proposed approach extends the concept of \gls{kme} by precisely identifying the logical relationships between generated responses and retrieved sentences, and selectively applying fine-tuning based on these relationships. This design aims to simultaneously satisfy the criteria of Locality and Generality.
...
First, the logical relationship—Entailment, Contradiction, or Neutral—between the model's output for existing sentences and newly retrieved sentences collected through \gls{rag} is evaluated. This process is performed automatically by leveraging a \gls{nli}-based framework.
...
In particular, selective fine-tuning is performed only when the relationship between the newly retrieved sentence and the existing output is identified as Contradiction or Neutral, while the existing output is preserved in the case of Entailment.
Furthermore, this study incorporates the model's confidence, measured by a softmax-based probability, into the relation assessment process.
The relation assessment is performed only when the model's confidence exceeds a predefined threshold (e.g., τ ≥ 0.7).
If the confidence falls below this threshold, the corresponding data are classified as Neutral, thereby preventing excessive updates or the incorporation of noise arising from uncertain judgments.
...
To validate the feasibility of the proposed \gls{raft} strategy, we applied it to the task of COVID-19-related fake news detection.
Conventional approaches relying solely on simple fine-tuning or prompt-based methods often face severe limitations in such tasks.
This is particularly evident in fake news detection, where temporal relevance is critical, since the veracity of information may change over time and conflicting facts frequently coexist.
...
Through social media (SNS) and the internet, information containing hallucinations as well as newly emerging data is being generated and disseminated at an explosive pace.
In such an environment, it is essential for models to incorporate the latest information in order to make consistent and reliable judgments on rapidly evolving issues such as COVID-19.
Although the global spread of COVID-19 has somewhat subsided, the possibility of similar public health crises reoccurring in the future remains high.
Accordingly, research on COVID-19 fake news detection continues to be actively pursued, and fake news detection systems must move beyond static, single-point modeling to incorporate structural flexibility and mechanisms for continuous updates.
This necessity becomes evident in the fact that models trained solely on initial datasets inevitably suffer degradation in both recency and accuracy over time.
...
Because COVID-19 is characterized by rapidly evolving information and the coexistence of conflicting facts at different points in time, fake news detection models require continuous updates.
However, indiscriminately incorporating all news into the model is not only inefficient but may also obscure decision boundaries and introduce confusion into the system.
...
In this way, the present study introduces a novel fine-tuning paradigm that is both efficient and flexible, complementing the limitations of existing approaches such as \gls{rag}, Continual Learning, and \gls{kme}, while remaining practically applicable.
The proposed \gls{raft} is not confined to COVID-19 fake news detection, but rather constitutes a strategic learning method that can be generalized to a wide range of domains where information evolves rapidly and consistency is of critical importance.
...
This paper is structured around two main contributions.
First, we designed a sentence-level detection model to establish a foundation for addressing time-sensitive fake news.
Second, we propose a \gls{raft} strategy that automatically evaluates the relationship between new sentences and existing outputs, and selectively performs fine-tuning based on the identified relation.
Through these contributions, the study overcomes the structural limitations of conventional static detection models and realizes a modeling framework that can flexibly adapt to dynamic information environments.
...
The proposed \gls{raft} is a framework that integrates FAISS-based semantic similarity retrieval, cross-encoder-based contextual re-ranking, and \gls{nli}-based relation classification.
In particular, the fine-tuning strategy is differentiated according to three relation types: entailment, contradiction, and neutral. Relation analysis is performed only when the model's softmax confidence exceeds a predefined threshold.
This design prevents excessive updates caused by uncertain data, enabling the effective incorporation of new information while preserving the consistency of existing knowledge.
...
In addition, this study structurally differentiates the loss function according to the identified relation type.
In particular, for the contradiction relation, a specialized loss function centered on False Loss was introduced to suppress prior incorrect outputs and facilitate the learning of new information.
Experimental results demonstrate that the proposed weighting scheme yields the most stable convergence curve, confirming its ability to achieve both efficiency and consistency in training.
...
Furthermore, to evaluate the balance between retaining prior knowledge and incorporating new knowledge, we quantitatively measured editing quality based on the Locality and Generality metrics, which have also been employed in approaches such as ROME and MEMIT.
In the Locality evaluation, an average of 93.4\% of 500 inputs unrelated to the edits preserved their original predictions, while in the Generality evaluation, an average of 91.0\% of 250 sentences semantically similar to the edited queries exhibited consistent directional changes in their outputs.
These results demonstrate that \gls{raft} can reliably edit the model without unnecessary parameter modifications.
...
In addition, comparative experiments with conventional fine-tuning validated the practical value of the proposed method.
Whereas standard fine-tuning required training on a large dataset of 30,000 samples, \gls{raft} achieved comparable accuracy (96.2\%) using only about 11,832 relation-based sentences.
This finding suggests that the proposed approach is far more suitable for real-world applications where hardware resources and time are severely constrained.
Consequently, this study presents a comprehensive and flexible learning strategy that not only integrates the strengths of existing approaches but also addresses their structural limitations.
...
To date, most studies have assumed a static learning-inference structure, in which the model is either fully retrained whenever new information becomes available or its existing parameters are completely replaced.
Such approaches face fundamental limitations in terms of real-time responsiveness and cost efficiency, which constitute the central motivation behind the Relation-Aware Fine-Tuning strategy proposed in this study.
Compared with conventional methods, the proposed \gls{raft} offers the following advantages:

\begin{itemize}
	\item{\textbf{1}:
	%전체 데이터를 재학습하지 않고도, 새로운 정보와 기존 지식 간의 논리적 관계에 기반한 최소한의 업데이트만으로 최신성을 유지할 수 있다.}
    Without retraining on the entire dataset, the model can maintain recency by performing only minimal updates based on the logical relationship between new information and existing knowledge.}
	\item{\textbf{2}:
	%기존 지식을 보존하면서도 새로운 지식을 효과적으로 반영함으로써, 지식 유지와 갱신 간의 균형을 달성한다.}
    By preserving prior knowledge while effectively incorporating new information, the model achieves a balance between knowledge retention and update.}
	\item{\textbf{3}:
	%연산 자원과 학습 시간을 크게 절감할 수 있으며, 특히 대규모 언어모델에서 효율적인 유지 관리가 가능하다.}
    It substantially reduces computational resources and training time, enabling efficient maintenance particularly in large-scale language models.}
	\item{\textbf{4}:
	%검색 기반이나 프롬프트 기반 방식에서 흔히 발생하는 논리적 비일관성을 최소화하여 데이터 정합성을 확보한다.}
    It minimizes the logical inconsistencies commonly observed in retrieval-based or prompt-based approaches, thereby ensuring data consistency.}
\end{itemize}
